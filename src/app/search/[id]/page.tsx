...
export default async function Search(props: { params: Promise<{ id: string }> }) {
  ...
  const postsRaw = await (sql`
    SELECT * FROM fetchSearchPosts(${id},10, 0);
  `) as {
  id: string;
  title: string;
  content: string;
  ...
  return (
    <div className="block w-full h-screen overflow-y-scroll scrollbar scrollbar-track-background/0 scrollbar-thumb-gray-600">
      <Posts id={id} userdata={userdata} courses={courses} posts={posts}/>
    </div>
  );
}