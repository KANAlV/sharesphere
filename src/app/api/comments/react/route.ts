...
export async function POST(req: Request) {
  ...
  const { commentId, action } = body; // "like" or "dislike"
  ...
  // --- Fetch the current comment reaction JSON ---
  const result = await sql`
    SELECT lnd FROM comments WHERE id = ${commentId}
  `;
  ...
  if (action === "like") {
    if (lnd.likes[userId]) {
      ...
      lnd.likes[userId] = true; // store userId as key
    }
  }
  ...
  if (action === "dislike") {
    if (lnd.dislikes[userId]) {
      ...
      lnd.dislikes[userId] = true; // store userId as key
    }
  }
